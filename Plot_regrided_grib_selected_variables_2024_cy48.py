...
from read_dble_cy48t1 import read_dble, read_dble_45_param, datehr_v, intV, Array_lon_lat, extract_time_V
import glob as glb
import copy as cpy
import cfgrib
...
for datei in fle_names:
        for mbN in range(T_mem):
                for f_ti in range(2, forecast_time_Mx): 
                        ...
                        ssrd_750m = dat_g[4]['hpbl'].data
                        lat_750m = dat_g[4]['hpbl']['latitude'].data
                        lng_750m = dat_g[4]['hpbl']['longitude'].data
                        del dat_g;
                        ...
                        date_v = dt(int(datei[-16:-6][0:4]), int(datei[-16:-6][5:7]),int(datei[-16:-6][8:10]))
                        ...
                        var_arr = np.reshape(fields_param[0], (tme_N, mem_N));
                        ssrd_vtx_t = var_arr[f_ti,mbN].data
                        
                        #ssrd_vtx_t = dat_zoom.data; qjfkjqfj
                        lat_1k3 = lat_a; #dat_vtx_t[20]['hpbl']['latitude'].data
                        lng_1k3 = lon_a; #dat_vtx_t[20]['hpbl']['longitude'].data
                        
                        #del dat_vtx_t;
                        
                        ssrd_1k3 =  ssrd_vtx_t ###--m
                        del ssrd_vtx_t;

                        fle_nme_png = grib_dir+'plots/'+dir_sfx.replace('/','_')+mb_str+'_'+fxi_str+'_hpbl.png'
                        fig = plt.figure(1, figsize =(9, 8))
                        plt.subplot(1,2,1)
                        plt.pcolor(lng_1k3,lat_1k3, ssrd_1k3, cmap=plt.cm.get_cmap('jet',15), vmin=1000, vmax=3000); plt.colorbar()
                        plt.xlim([0, 4]); plt.ylim([47.75, 50]);
                        plt.title(suite+st_res+' from vortex')
                        plt.subplot(1,2,2)
                        plt.pcolor(lng_750m,lat_750m, ssrd_750m, cmap=plt.cm.get_cmap('jet',15), vmin=1000, vmax=3000); plt.colorbar()
                        plt.xlim([0, 4]); plt.ylim([47.75, 50])
