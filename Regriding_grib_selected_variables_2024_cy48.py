...
T_mem = 24 ###--24--ensemble members
forecast_time_Mx = 51 ###--51--Forecast terms
		...
		if suite=='dble' or 'H3Z3':
			for mi in range(mem_N):
				...
				for ti in range(0,tme_N):
                                        ...
                                        for sel_var in fields_param:
                                                ###-------Selection of a Variable-----------
                                                var_arr = np.reshape(sel_var, (tme_N, mem_N));
                                                srf_cld_zm = var_arr[ti,mi].extract_zoom(dict(lonmin=0.67,lonmax=4.02,latmin=47.75,latmax=49.95))
                                                srf_cld_zm.geometry
                                                ...
                                                ###-------Writing a grib file for the selected variables-----------
                                        	
                                                wr.writefield(srf_cld_zm)
                                                del srf_cld_zm
                                        wr.close()
                                        toc_h = time.time();
                                        print('Writing re-grided Grib at '+grib_dir_out +' and is Done in {:.4f} seconds'.format(toc_h-tic_h));
                                        del wr, grib_dir_out, TmpFle
					###-----------------
