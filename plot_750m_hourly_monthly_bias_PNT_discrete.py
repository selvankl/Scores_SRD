...
tme_end = 20 ###---Limit the time to day by considering the solar zenith angle

### ========--- READING PEAROME at 1.3 km of cy48t1----===============
suite = "H3Z3"; suite_a = "H1EK";
...
def stat_collect(h, nc_dir, fle_str_sfx, str_l_path):
	...
	str_hr = "*"+hr_st+"_mem"
	fle_name_nc = glb.glob(nc_dir+"*"+str_hr+fle_str_sfx); print(fle_name_nc)
...
if __name__ == "__main__":
	...
	case_cld = {}
	case_cld["HIT"] = {'name':'HIT at 500 m', 'color':'brown'}
	case_cld["FALSE ALARM"] = {'name':'FALSE ALARM at 500 m', 'color':'g'}
	case_cld["MISSED"] = {'name':'MISSED at 500 m', 'color':'b'}
	case_cld["CLEAR SKY"] = {'name':'CLEAR SKY at 500 m', 'color':'r'}
	
	case_cld["HIT m"] = {'name':'HIT at oper', 'color':'chocolate'}
	case_cld["FALSE ALARM m"] = {'name':'FALSE ALARM at oper', 'color':'darkviolet'}
	case_cld["MISSED m"] = {'name':'MISSED at oper', 'color':'darkslategrey'}
	case_cld["CLEAR SKY m"] = {'name':'CLEAR SKY at oper', 'color':'deeppink'}
	...
	plt.xticks([r + barWidth for r in range(len(xax_lbl))], xax_lbl)
	#plt.legend()
	fle_out_png = suite_a+'_'+suite+'_Comparison_Hourly_'+PNT+"_" + run_UT + "_SWD_Bias_" + (date_lh['6'].min()).strftime('%Y%m%dT%H')+ "_" + (date_lh['6'].max()).strftime('%Y%m%dT%H') + "_K"+str(K)+"_"+str(fn_lim)+ "h_hist.png";
	...
	fle_out_png = suite_a+'_'+suite+'_Comparison_Hourly'+PNT+"_" + run_UT + "_Frequency_" + (date_lh['6'].min()).strftime('%Y%m%dT%H')+ "_" + (date_lh['6'].max()).strftime('%Y%m%dT%H') + "_K"+str(K)+"_"+str(fn_lim)+ "h_hist.png";
	fig.savefig(png_dir+ fle_out_png, dpi = 300, bbox_inches='tight')
	plt.close(fig)  
	...
	plt.xticks([r + barWidth for r in range(len(xax_dble_MN))], xax_dble_MN, rotation=15); plt.xlim([-0.5, 12.75]);
	#plt.legend()
	fle_out_png = suite_a+'_'+suite+'_Comparison_monthly_'+PNT+"_" + run_UT + "_SWD_Bias_" + (date_lh['6'].min()).strftime('%Y%m%dT%H')+ "_" + (date_lh['6'].max()).strftime('%Y%m%dT%H') + "_K"+str(K)+"_"+str(fn_lim)+ "h_hist.png";
	fig.savefig(png_dir+ fle_out_png, dpi = 300, bbox_inches='tight')
	plt.close(fig)  
	...
	plt.xticks([r + barWidth for r in range(len(xax_dble_MN))], xax_dble_MN, rotation=15); plt.xlim([-0.5, 12.75]);
	plt.legend(loc=1, fontsize = 5, ncol=2)
	fle_out_png = suite_a+'_'+suite+'_Comparison_monthly_'+PNT+"_" + run_UT + "_Frequency_" + (date_lh['6'].min()).strftime('%Y%m%dT%H')+ "_" + (date_lh['6'].max()).strftime('%Y%m%dT%H') + "_K"+str(K)+"_"+str(fn_lim)+ "h_hist.png";
	fig.savefig(png_dir+ fle_out_png, dpi = 300, bbox_inches='tight')
	plt.close(fig)  
