...
date_temp1 = '20240911' # sys.argv[1]
date_temp2 = '20240911' # sys.argv[2]
y1 = int(date_temp1[:4])
...
forecast_time_Mx = 21 ###----Forecast terms
...
def scores_cases_BDclim_zone_M_N_lon_lat_opt(csd_tot, mem_N, nebul_cems_h, lon, lat, BD_data_dir, num_station, csd_arome_st, swd_m_l, lon_a, lat_a, M, N, K, h):
	...
	if csd_tot == mem_N:
		...
		Z_zone = []
		swd_m_zone = swd_m[:,M_i_:M_ik,N_j_:N_jk]
		lon_zone = lon_a[M_i_:M_ik,N_j_:N_jk]
		lat_zone = lat_a[M_i_:M_ik,N_j_:N_jk]
		Z_zone.append( swd_m_zone )
		Z_zone.append( lon_zone )
		Z_zone.append( lat_zone )
		print(swd_m_zone.shape, lon_zone.shape, lat_zone.shape); 
		del lat_zone, lon_zone, swd_m_zone, M, M_i_, M_ik, N, N_j_, N_jk;
		#--------------------------------------------------
		...
		#---print(Z, v_csd)
	return [ghi_b, csd_b, csd_cems_h, Z_zone, TOA_estimation]
	...
	for d in range(0,num_jour):
		...
		if suite=='dble':
			for mi in range(mem_N):
				...
				ssrd_zoom_0.geometry; jjqkgkljl
		...
		mbi = 0;
		date_mb0_fterm = date_arr_A[1:,mbi] 
		h_min = 6
		h_term = np.arange(6,forecast_time_Mx,1)
		h_max_t = 20
		
		for h in range(h_min, h_max_t):
			run_date = [tij for tij in date_mb0_fterm if tij.hour==h][0]
			...
			#--------------------------------------------			
			print(h); csd_arome_st = np.nanmean(csd_arome_h, axis=1)
			...
			if good_stat_ID.shape[0] != 0:
				...
				for i in good_stat_ID:
					...
					Z = cems_csd_scores_l[3][0]  ###-----Deterministic
					Z_tot = (np.median(Z, axis=0)).flatten()
